...
class _ShelfDetailViewState extends State<ShelfDetailView> {
  ...
  Future<void> _reloadShelf() async {
    final updatedShelf =
        await FirebaseShelfService().getShelfById(widget.shelf.id);
    if (updatedShelf != null) {
      setState(() {
        widget.shelf.assignedProducts = updatedShelf.assignedProducts;
        widget.shelf.productsCount = updatedShelf.productsCount;
      });
    }
  }
